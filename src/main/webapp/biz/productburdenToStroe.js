	...
	$.post(
			...
			function(data){
				...
				for(var i = 0;i < data.listDatas.length;i++){
					...
					sumitemid+= "<li>"
								sumitemid+=	" <a href='Product_Burden.jsp'><button class='btn btn-info btn-sm' >物料需求表</button></a>"          
					sumitemid+="</li>"
						
					sumitemid+= "<li>"
					if(data.result==true){
						sumitemid+=	"<a href='#' onclick='checkid()' class='btn btn-info btn-sm' >确定出货</a>"          
					}else{
						sumitemid+="<a href='#' onMouseOver='checkid2()' >确定出货</a>"
					}
					sumitemid+="</li>"
					sumitemid+="</ul>"
...
function checkid(){
	...
	location.href="queryDemandToProduct.do?wid="+id;
	
}

function checkid2(){
	alert("对不起不能出库，请核对好再出库!");
