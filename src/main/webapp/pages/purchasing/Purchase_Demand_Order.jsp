...
//		拼接
		...
		for (var i = 0; i < pdo.listDatas.length; i++) {

			code += "<tr align='center'>";
			code += "<td>" + pdo.listDatas[i].demandOrderId + "</td>";
			code += "<td>" + pdo.listDatas[i].demandDate + "</td>";
			code += "<td>" + pdo.listDatas[i].fillEmployee + "</td>";
			if (pdo.listDatas[i].checkStatus == 1) {
				code += "<td>未审核</td>";
			}
			if (pdo.listDatas[i].demandStatus == 1) {
				code += "<td>未制定采购计划</td>";
			}
			code += "<td><button type='button' class='btn btn-danger btn-sm' onclick='dele2(\""+pdo.listDatas[i].demandOrderId+"\")'>驳回</button>";
			code += "<button type='button' class='btn btn-info btn-sm' onclick=javasrcipt:location.href='Purchase_add_Plan_Order.jsp?id="+pdo.listDatas[i].demandOrderId+"'>通过</button></td>";
			code += "</tr>";
...
function dele2(demandOrderId) {
	
	//	alert("ces="+demandOrderId);
	$.post(
		"PurchaseDemand.do", 
...
<body onload="query()" class="skin-blue">
	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
		...
		<h1>
			采购管理 <small>采购需求单管理</small>
		</h1>
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> 主页</a></li>
			<li><a href="#">采购管理</a></li>
			<li class="active">采购需求单</li>
